...
object CreditSMEventAdapter
  ...
  override def fromJournal(event: CreditSM.PersistEvent, manifest: String): EventSeq[CreditSM.PersistEvent] =
    event match {
      case SalesManagerApprovalPersistEvent(controlObject) => {
        val creditTenants:  java.util.List[Customer]  = controlObject.get("creditTenants")
          .asInstanceOf[java.util.List[Customer]]
        ...
        controlObject.put("creditTenants", customerV2Tenants)
        EventSeq.many(
          SomeAdditionalManagerApprovedPersistEvent(controlObject),
          SalesManagerApprovalV2PersistEvent(controlObject))
      }
