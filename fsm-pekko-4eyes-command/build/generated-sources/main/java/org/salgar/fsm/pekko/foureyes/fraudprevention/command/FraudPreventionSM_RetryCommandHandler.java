...
@Slf4j
public class FraudPreventionSM_RetryCommandHandler implements CommandHandler<FraudPreventionSMCommand, FraudPreventionSM.Response> {
    ...
    public Future<FraudPreventionSM.Response> handleCommand(FraudPreventionSMCommand fraudPreventionSMCommand) {
        return fraudPreventionSMFacade.askRetry(() -> fraudPreventionSMCommand.getUseCaseKey(), convertProtobuf2Pojo(fraudPreventionSMCommand.getPayloadMap()));
    }

    @Override
    public String type() {
        return FraudPreventionSMCommandConstants.ON_RETRY ;
    }
