...
@Slf4j
public class FraudPreventionSM_ErrorCommandHandler implements CommandHandler<FraudPreventionSMCommand, FraudPreventionSM.Response> {
    ...
    public Future<FraudPreventionSM.Response> handleCommand(FraudPreventionSMCommand fraudPreventionSMCommand) {
        return fraudPreventionSMFacade.askError(() -> fraudPreventionSMCommand.getUseCaseKey(), convertProtobuf2Pojo(fraudPreventionSMCommand.getPayloadMap()));
    }

    @Override
    public String type() {
        return FraudPreventionSMCommandConstants.ON_ERROR ;
    }
